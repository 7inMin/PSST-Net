...
class RealSSLT93(nn.Module):
    """Real CASSI interface: convert coded Y/Phi before the SSLT_93 backbone."""

    def __init__(self, bands=28, step=2, input_resolution=384):
        super().__init__()
        self.bands = bands
        self.step = step
        self.backbone = SSLT(
    ...
    def shift_back_y(self, y):
        if y.ndim != 3:
            raise ValueError(f"Y must have shape [B,H,W+54], got {tuple(y.shape)}")
        width = y.shape[-1] - (self.bands - 1) * self.step
        if width <= 0:
            raise ValueError(f"Invalid coded measurement width: {y.shape[-1]}")
        return torch.stack(
            [y[:, :, self.step * band:self.step * band + width] for band in range(self.bands)],
            dim=1,
        )

    def shift_back_phi(self, phi, width):
        if phi.ndim != 4 or phi.shape[1] != self.bands:
            raise ValueError(f"Phi must have shape [B,{self.bands},H,W+54], got {tuple(phi.shape)}")
        return torch.stack(
            [phi[:, band, :, self.step * band:self.step * band + width] for band in range(self.bands)],
            dim=1,
        )

    def forward(self, y, phi, return_stages=False):
        h = self.shift_back_y(y)
        mask = self.shift_back_phi(phi, h.shape[-1])
        return self.backbone(h, mask, return_stages=return_stages)
