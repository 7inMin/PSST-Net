...
def shuffle_crop(train_data, batch_size, crop_size=256, argument=True):
    ...
    if batch_size == 1:
        ...
        processed_data = np.zeros((1, crop_size, crop_size, 28), dtype=np.float32)
        processed_data[0, :, :, :] = img[x_index:x_index + crop_size, y_index:y_index + crop_size, :]
        processed_data = torch.from_numpy(np.transpose(processed_data, (0, 3, 1, 2))).cuda().float()
        gt_batch = [arguement_1(processed_data[0])]
...
def gen_meas_torch(data_batch, mask3d_batch, Y2H=False, mul_mask=False):
